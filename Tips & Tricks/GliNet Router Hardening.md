...
!!!warning
Feel free to change other settings like server tokens or adding headers like HSTS, which would prevent the browser from ever connecting over HTTP. However, beware that an update could revert these changes, which would force you to clear HSTS in your browser. If you want to modify  ciphers, have a look at [!ref target="blank" text="Mozillas SSL config generator"](https://ssl-config.mozilla.org/#server=nginx&version=1.17.7&config=intermediate&openssl=1.1.1m&hsts=false&ocsp=false&guideline=5.7).
!!!
